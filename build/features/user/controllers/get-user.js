...
class Get {
  ...
  async feed(req, res) {
    ...
    if (cachedData) {
      ...
      const frequentlyPurchasedProducts =
        await product_service_1.productService.getFrequentlyPurchasedProductsNearUser([long, lat], 10);
      feedData.push({
  ...
  async profile(req, res) {
    const { userId } = req.params;
    const user = await user_service_1.userService.getUserById(userId);
    if (!user) throw new error_handler_1.NotFoundError('Account not found');
    res.status(http_status_codes_1.default.OK).json({ message: 'User Profile', user });
  }
  async savedStores(req, res) {
    const user = await (
      await user_service_1.userService.getUserById(req.currentUser.userId)
    ).populate('savedStores', '-owner');
    res
      .status(http_status_codes_1.default.OK)
      .json({ message: 'Saved stores', savedStores: user.savedStores });
  }
  async likedProducts(req, res) {
    ...
    res
      .status(http_status_codes_1.default.OK)
      .json({ message: 'Liked products', likedProducts: user.likedProducts });
  }
}
