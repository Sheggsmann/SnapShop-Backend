...
class UpdateOrder {
  async confirmOrderPayment(req, res) {
    ...
    if (!order) throw new error_handler_1.BadRequestError('Could not confirm order');
    const amountToPay = helpers_1.Helpers.calculateOrderTotal(order);
    res
      .status(http_status_codes_1.default.OK)
      .json({ message: 'Order confirmed successfully', order, amountToPay });
  }
      ...
      if (eventData.event === 'charge.success') {
        ...
        if (order) {
          ...
          if (total === amountPaid) {
            ...
            await order_service_1.orderService.updateOrderPaymentStatus(
              orderId,
              true,
              amountPaid,
              deliveryCode,
            ...
            product_queue_1.productQueue.addProductJob('updateProductPurchaseCount', {
  ...
  async devOrderPayment(req, res) {
    ...
    if (order) {
      ...
      if (total !== amountPaid) throw new error_handler_1.BadRequestError('Incorrect amount');
      // Subtract service fee from the amount paid and credit to store
  ...
  async order(req, res) {
    ...
    const order = await order_service_1.orderService.getOrderByOrderId(orderId);
    if (!order) throw new error_handler_1.NotFoundError('Order not found');
    ...
    if (isOrderStore) {
      notification_queue_1.notificationQueue.addNotificationJob('sendPushNotificationToUser', {
        ...
        value: {
          title: 'Order Updated',
          body: `${order.store.name} added a delivery fee for order #${order._id.toString().substring(0, 8)}`
        }
      });
    }
